...
for filename in arr:
   if filename.startswith('CommitmentDetails'): 
...
zipf = zipfile.ZipFile( config["Path"]["Archive_TRD"]+"\\"+"TradeManagement_"+date+".zip" , 'w', zipfile.ZIP_DEFLATED)
zipdir( config["Path"]["Inbound_TRD"] , zipf)
zipf.close()

for filename in list_processed:
    os.remove(config["Path"]["Inbound_TRD"]+"/"+filename )
