...
#               filenames from Inbound diectory and writes an entry in "XML_LSM_SourceFileList" as OPEN for 
...
#               batches are updated as FAILED or COMPLETED based on run status in table "XML_LSM_SourceFileList"
# -------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------
# Description : Read the XML file names from Inbound and write into XML_LSM_SourceFileList as open 
#               for parsing 
# ------------------------------------------------------------------------------------------------
    ...
    def BatchOpen(self):
        ...
        logging.info("Batch Table  XML_LSM_SourceFileList Insert:")
        ...
        try:
            ...
            for filename in list_processed:
                logging.info("Checking for the status of the file " +filename +" in 'XML_LSM_SourceFileList' table ")
                ...
                if rowcount==0: 
                    ...
                    logging.info("Inserting to 'XML_LSM_SourceFileList' table as batch OPEN "+filename)
                    ...
                    for row in result:
                        ...
                        # Check for existing batch: If failed or not completed
                            ...
                            logging.info("Either IICS flag or Python flag is not completed, hence updating the batch file to OPEN " +filename)
            ...
            logging.info("Insert/Update to XML_LSM_SourceFileList table based on Batch Status successful")
        
        except Exception as error:
            
            logging.error("Listing and inserting LoanMetadata files to XML_LSM_SourceFileList caught with exception %s"+str(error))
            raise

# ------------------------------------------------------------------------------------------------
# Description : Read the XMLFilenames from the XML_LSM_SourceFileList which are "OPEN" for parsing
...
# ------------------------------------------------------------------------------------------------
    ...
    def ProcessMetadataXmlFileList(self):
        ...
        logging.info("Processing the LoanMetadata xml files in XML_LSM_SourceFileList table status as OPEN.")
        
        try:
            
            logging.info("Get the filelist,BatchID which are batch OPEN from XML_LSM_SourceFileList table and loop:")
...
# ------------------------------------------------------------------------------------------------
# Description : Parse the xml files which are listed in XML_LSM_SourceFileList table taking the 
...
# ------------------------------------------------------------------------------------------------
    ...
    def ParsingLoanMetaDataXml(self,filename,BatchID,CreateTs):
        ...
        try :
            ...
            for i in root.iter():
                for clientid in i.findall("ClientID"):
                    ClientID=clientid.text

                for clientname in i.findall("ClientName"):
                    ClientName=clientname.text

                for productid in i.findall("ProductId"):
                    ProductId=productid.text

                for loanid in i.findall("LoanId"):
                    LoanId=loanid.text

                for loannumber in i.findall("LoanNumber"):
                    LoanNumber=loannumber.text
            ...
            # -----------------------------------------------------------------
            # LDM_XML_DocData_Stg Load: -Insert 2(For Missing indicators)
            #                  
...
# ------------------------------------------------------------------------------------------------
    ...
    def BatchFailed(self,filename):
        ...
        logging.info("Started updating the batch failed in XML_LSM_SourceFileList for file : "+ filename)
        
        try:
            #Update the processed flag to failed for the file failed in the current run 
            ...
            logging.info("Updating the batch to failed in XML_LSM_SourceFileList successfull for file : "+filename)
        
        except Exception as e:

            logging.error("Exception while Updating the batch to failed in XML_LSM_SourceFileList for file: "+filename+"Error: "+str(e))
...
# ------------------------------------------------------------------------------------------------
    ...
    def BatchUnprocessed(self):
        ...
        try:
            logging.info("Get the LoanMetaData filelist which are batch FAILED from XML_LSM_SourceFileList table :")
            
            #Checck for the failed batch in XML_LSM_SourceFileList
...
if __name__ == '__main__':
    ...
    server = ConfigParser()
    server.read('C:\\Users\\informatica\\lmp_support\\LoanLogics\\ODBC.ini')
