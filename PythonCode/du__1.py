                ...
                if filename.startswith(config["DesktopUnderwriter"]["File_Prefix"]):
                    ...
                    if os.path.isfile(full_path) and filename not in idList:
                        ...
                        if FileName in idList_unprocessed:
                            cursor.execute("UPDATE "+server["SqlServer_Connection"]["DBname"]+"."+server["SqlServer_Connection"]["schema_name"]+"."+config["DB_ObjectNames"]["tb_dumessasges"]+" SET ISPROCESSED_FLAG= 'Y' WHERE FILENAME = "+Filename)
                            cursor.commit()
                        else:                    
